...
export const metadata = { title: "IT Services | Astelfin", robots: { index: false, follow: false } };

const DEPT_ALIASES = ["IT", "IT Services", "Information Technology", "Systems", "Technology"];

const QUICK_ACTIONS = [
  { label: "Asset Register",     desc: "IT equipment and hardware register",       href: "/astelfin_26/assets",       icon: "🖥️" },
  { label: "Procurement",        desc: "Purchase requests and vendor quotes",       href: "/astelfin_26/procurement",  icon: "🛒" },
  { label: "Document Library",   desc: "Technical documents and SOPs",             href: "/astelfin_26/documents",    icon: "📂" },
  { label: "Settings",           desc: "Platform and system configuration",         href: "/astelfin_26/settings",     icon: "⚙️" },
  { label: "Audit Log",          desc: "System activity and access log",            href: "/astelfin_26/settings/audit",icon: "🔍" },
  { label: "Compliance",         desc: "Data and security compliance checks",       href: "/astelfin_26/compliance",   icon: "🛡️" },
];
...
export default async function ITDeptPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10">
      <div className="space-y-1">
        ...
        <h1 className="text-3xl font-bold text-brand-navy">IT Services</h1>
        <p className="text-brand-muted text-sm">Technology infrastructure, systems management and IT support for Astelfin and its clients.</p>
      ...
      {!org && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 text-sm text-amber-800">
          <strong>Setup required:</strong> Create an Organisation with shortCode <code>ASTELFIN</code>.
      ...
      {org && (
        <section className="space-y-6">
          <h2 className="text-xs font-bold uppercase tracking-widest text-brand-muted">IT Team</h2>
          {staff.length === 0 ? (
            <div className="bg-white rounded-2xl border-2 border-dashed border-gray-200 p-12 text-center">
              <p className="text-2xl mb-2">🖥️</p>
              <p className="font-semibold text-brand-navy">No IT staff yet</p>
              <p className="text-sm text-brand-muted mt-1">Add employees with the <strong>IT Services</strong> department tag.</p>
      ...
      <section className="space-y-4">
        <h2 className="text-xs font-bold uppercase tracking-widest text-brand-muted">IT Operations</h2>
